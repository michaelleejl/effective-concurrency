...
let is_value (e, _) = match e with Skip | Num _ -> true | _ -> false

let rec schedule (es : config Queue.t) =
  if Queue.is_empty es then ()
  else
...
let run_process (es, i) =
  let s = ref (fun _ -> 0) in
  (*all locations initialised to zero*)
  let m = ref (fun _ -> None) in
  (* all locks unlocked *)
  let z = ref (fun _ -> 0) in
  (* all semaphores set to zero *)
